...
- [ ] Implement remarquee upload md: accept markdown files and directories (directories recursively scanned for *.md), preprocess markdown (strip docmgr frontmatter + list normalization), run pandoc/xelatex, and upload via rmapi
- [ ] Implement recursive remote mkdir helper (mkdir -p) for /ai/YYYY/MM/DD/... (and user-provided remote directories)
