...
function remarqueeChurnMetrics(filters) {
  ...
  const filtered = stats.filter((s) => Number(s.remq_call_count) >= filters.min_remq_calls);
  ...
  return filtered
    .map((s) => {
      const remqCalls = Number(s.remq_call_count);
      const remqSuccesses = Number(s.remq_success_count);
      const remqFailures = Number(s.remq_fail_count);
      const totalBash = Number(s.total_bash_calls);
      const totalTools = Number(s.total_tool_calls);
      const totalTurns = Number(s.total_turns);
